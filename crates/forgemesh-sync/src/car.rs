use forgemesh_core::{store::ForgeStore, types::DataNode};
...
pub enum CarError {
    ...
    Storage(#[from] forgemesh_core::store::StorageError),
    #[error("Invalid magic")]
    InvalidMagic,
}
        ...
        loop {
            ...
            match store.get(&hash)? {
                ...
                None => {
                    ...
                    let current_latest = store.get_history(&node.sensor_id, 1).ok().and_then(|v| v.first().map(|n| n.data_hash.clone()));
