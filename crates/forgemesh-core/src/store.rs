...
impl ForgeStore {
    ...
    pub fn put(&self, sensor_id: &str, value: f64, timestamp_ns: u64) -> Result<String, StorageError> {
    ...
    pub fn get_history(&self, sensor_id: &str, limit: usize) -> Result<Vec<DataNode>, StorageError> {
    ...
    pub fn update_index(&self, sensor_id: &str, hash: &str) -> Result<(), StorageError> {
        self.db.insert(format!("idx:{sensor_id}").as_bytes(), hash.as_bytes())?;
        Ok(())
    }
}
